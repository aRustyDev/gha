...
# verifying attestation chains, and detecting changed artifacts.
#
# Usage:
#   source scripts/attestation-lib.sh
#
# Required environment variables:
#   GITHUB_REPOSITORY - The owner/repo (e.g., aRustyDev/repo)
#
# Optional environment variables:
#   GH_TOKEN or GITHUB_TOKEN - For API authentication
#   ARTIFACT_PATH - Path pattern for artifacts (default: "charts")
...
#######################################
# Detect artifacts that changed in a commit range
#
# Generic function that works with any artifact directory structure.
# Looks for directories containing a manifest file (e.g., Chart.yaml, Cargo.toml).
#
# Arguments:
...
#   $3 - artifact_path: Path to artifacts (optional, uses ARTIFACT_PATH env var)
#   $4 - manifest_file: Manifest file name (optional, uses MANIFEST_FILE env var)
#
# Outputs:
#   Space-separated list of artifact names that changed
#######################################
detect_changed_artifacts() {
    ...
    for artifact in $artifacts; do
        if [[ -f "${artifact_path}/${artifact}/${manifest_file}" ]]; then
            ...
            echo "::notice::Skipping $artifact - no $manifest_file found" >&2
        fi
    done

    valid_artifacts=$(echo "$valid_artifacts" | xargs)  # Trim whitespace

    if [[ -z "$valid_artifacts" ]]; then
        echo "::notice::No artifact changes detected in range: $range" >&2
    else
        echo "::notice::Changed artifacts: $valid_artifacts" >&2
    fi

    echo "$valid_artifacts"
}

# Alias for backward compatibility with Helm-specific code
detect_changed_charts() {
    local range="${1:-HEAD~1..HEAD}"
    local base_branch="${2:-}"
    detect_changed_artifacts "$range" "$base_branch" "charts" "Chart.yaml"
}
...
#
# Reads a CHANGELOG.md file and extracts the section
...
extract_changelog_for_version() {
    local artifact="$1"
    local version="$2"
    local artifact_path="${3:-${ARTIFACT_PATH:-charts}}"
    local changelog_file="${artifact_path}/${artifact}/CHANGELOG.md"

    if [[ -z "$artifact" || -z "$version" ]]; then
        echo "No changelog available (missing artifact or version)"
        return 0
    fi

    if [[ ! -f "$changelog_file" ]]; then
        echo "No changelog available for $artifact"
        return 0
    fi
...
#######################################
# Get version from a manifest file
#
...
# Arguments:
#   $1 - artifact: Artifact name
#   $2 - artifact_path: Path to artifacts (optional, uses ARTIFACT_PATH env var)
#   $3 - manifest_file: Manifest file name (optional, uses MANIFEST_FILE env var)
#
# Outputs:
#   Version string
#######################################
...
export -f detect_changed_artifacts
